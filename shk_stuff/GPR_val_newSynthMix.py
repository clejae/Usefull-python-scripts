...
wd = r'B:\temp\temp_clemens\GPR'
out_files = r'\regressors\levels\out_' + regression_type   # '\tests\test2\EnMAP_' #wd + out_files + season
...
seasonslist = ['spsufa']
levellist = ['3b']
val_site_ignorelist = ['A_38','A_39','A_56','A_57','A_64','C_41','C_05','C_06','C_16','C_39']
        #No ag or urban sites: ['A_38','A_39','A_56','A_57','A_64','C_41','C_5','C_6','C_16','C_39']
...
classlists = {'1b': ['Unclassified', 'Veg', 'Back'],
        ...
        '2': ['unclassified', 'WoodyVeg', 'NonWoodyVeg', 'Imperv', 'Water'],
...
def synthMixRegressionWorkflow(loop, classID):
    ...
    ## - import ENVI speclib
    unsupervisedSample = ENVISpectralLibrary(filename=speclibFilename)

    ## - label spectra
    classificationSample = ClassificationSample(raster=unsupervisedSample.raster(),
                                                classification=Classification.fromRasterMetadata(
                                                filename='/vsimem/labels.bsq',
                                                raster=unsupervisedSample.raster(),
                                                classificationSchemeName='level '+str(level)))
    ...
    fractionSample = classificationSample.synthMix2(
        ...
        n=n_synthmix, target=classID, includeWithinclassMixtures=True, includeEndmember=False)
    ...
    end_time = time.time()
    print(classes[classID] + ' Loop '+str(loop) + " took {} minutes to execute this".format(end_time - start_time)/60.)
...
class RegressorGPR(Regressor):
    ...
    def predictStd(self, filename, image, mask=None, **kwargs):
        ...
        applier.apply(operator=_RegressorGPRPredictStd, image=image, estimator=self, mask=mask)
...
for cur_level in levellist:
    ...
    for iteration in seasonslist:
        ...
        out_dir = wd + out_files + season + r'\\' + lib + r'_' + level + param + r'\\'
        stats_out_dir = wd + out_files + season + r'\stats\\'
        stack_out_dir = wd + out_files + season + r'\stacks\\'
        synthmix_out_dir = wd + out_files + season + r'\synth\\'

        refdat = np.genfromtxt(r"B:\temp\temp_sam\GPR\data\reference\ABCD_validation2.csv", delimiter=',', dtype=str)
        speclibFilename = wd + r'\data\spectrallibrary\seasons\EnMAP' + season + '_' + lib + '.sli'  # shape\puritycheck_v2b.sli'
        enmapFilename = wd + r'\data\image\val_img\ABCD_' + season + '.bsq'

        speclibFilename = r"B:\temp\temp_sam\GPR\tests\00_speclib\temporal check\00_new_speclib.sli"
        ...
        if level == '1b':
            ...
            data_ref_all[:, 1] = refdat[1:, 14]       # l2_wVeg
            data_ref_all[:, 2] = refdat[1:, 15]       # l2_nwVeg
            data_ref_all[:, 3] = refdat[1:, 16]       # l2_back
        elif level == '3b':
            ...
            data_ref_all[:, 1] = refdat[1:, 20]       # l4_conifer
            data_ref_all[:, 2] = refdat[1:, 21]       # l4_broad
            data_ref_all[:, 3] = refdat[1:, 22]       # l4_shrub
            data_ref_all[:, 4] = refdat[1:, 23]       # l4_grass
            data_ref_all[:, 5] = refdat[1:, 24]       # l4_back
